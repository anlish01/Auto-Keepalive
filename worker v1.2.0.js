// HTML 前端模板 (Vue 3 + Tailwind CSS - 独立登录页面与 Session 机制完全体版)
const UI_HTML = `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <title>站点保活管理系统 - Pro v1.2.0</title>
    ...
    <div id="app" class="max-w-6xl mx-auto">
        ...
        <div v-if="!isLoggedIn" class="flex flex-col items-center justify-center min-h-[80vh] px-4 animate-fade-in">
            <div class="bg-white dark:bg-slate-800 p-8 sm:p-10 rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-700 max-w-md w-full text-center relative overflow-hidden">
                ...
                <p class="text-xs text-gray-400 dark:text-slate-400 mb-8">Auto-Keepalive Pro v1.2.0</p>

                <div class="space-y-4 text-left">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1.5 ml-1">管理员账号</label>
                        <input v-model="loginForm.user" @keyup.enter="doLogin" type="text" placeholder="请输入 ADMIN_USER" class="w-full p-3.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-900 text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1.5 ml-1">访问密码</label>
                        <input v-model="loginForm.pass" @keyup.enter="doLogin" type="password" placeholder="请输入 ADMIN_PASS" class="w-full p-3.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-900 text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition">
                    </div>
        ...
        <div v-else class="animate-fade-in">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700/80">
                <div>
                    <h1 class="text-2xl font-extrabold flex items-center gap-2.5 text-gray-900 dark:text-white">
                        <span>🌐</span> Auto-Keepalive <span class="text-xs px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 font-bold border border-indigo-200 dark:border-indigo-800">Pro v1.2.0</span>
                    </h1>
                    <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">Cloudflare Serverless 多站点脉搏监控与批量分发中枢</p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2.5 w-full md:w-auto justify-end">
                    ...
                    <button @click="doLogout" class="bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-950/40 dark:hover:bg-red-900/60 dark:text-red-400 border border-red-200 dark:border-red-800/80 px-3 py-2 rounded-xl transition font-bold text-xs flex items-center gap-1 ml-1" title="销毁当前会话令牌">
                        <span>🚪</span> <span class="hidden sm:inline">退出登录</span>
                    </button>
                </div>
            </div>
            ...
            <div class="flex border-b border-gray-200 dark:border-slate-700 mb-6 gap-2 overflow-x-auto pb-1">
                ...
                <button @click="currentTab = 'logs'" :class="currentTab === 'logs' ? 'bg-indigo-50 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-bold border-b-2 border-indigo-600 dark:border-indigo-400' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/50'" class="py-2.5 px-5 rounded-t-xl transition text-sm whitespace-nowrap flex items-center gap-1.5">
                    <span>📜</span> 运行日志
                ...
                <div v-if="currentTab === 'channels'" class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 max-w-4xl mx-auto space-y-6">
                    <div class="p-5 bg-gray-50 dark:bg-slate-900/60 rounded-2xl border border-gray-200 dark:border-slate-700">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <input v-model="newChannel.name" placeholder="设置一个渠道别名 (如: 主力TG群)" class="w-full p-3 border rounded-xl bg-white dark:bg-slate-800 dark:border-slate-700 text-sm font-medium">
                        </div>
            ...
            <div v-if="showBatchModal" class="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all duration-200 animate-fade-in">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full border border-gray-200 dark:border-slate-700 overflow-hidden">
                    <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
                        <h3 class="font-black text-base text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
                            <span>📦</span> 批量分配通知渠道
                        </h3>
                        <button @click="showBatchModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-white text-xl font-bold">&times;</button>
                    </div>
                    <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                        <div class="text-xs text-amber-700 dark:text-amber-300 font-bold bg-amber-50 dark:bg-amber-950/50 p-3 rounded-xl border border-amber-200 dark:border-amber-800 flex items-center gap-2">
                            <span>💡</span> 即将为已勾选的 {{ selectedTaskIndices.length }} 个保活任务执行批量同步
                        </div>

                        <div>
                            <label class="block text-xs font-extrabold text-gray-500 dark:text-slate-400 mb-2">选择覆盖逻辑</label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="flex items-center gap-2 p-3 rounded-xl border cursor-pointer font-bold text-xs select-none transition" :class="batchMode === 'overwrite' ? 'border-indigo-600 bg-indigo-50/50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400' : 'border-gray-200 dark:border-slate-700'">
                                    <input type="radio" value="overwrite" v-model="batchMode" name="bmode" class="accent-indigo-600">
                                    <span>完全替换 (抹除原有)</span>
                                </label>
                                <label class="flex items-center gap-2 p-3 rounded-xl border cursor-pointer font-bold text-xs select-none transition" :class="batchMode === 'append' ? 'border-indigo-600 bg-indigo-50/50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400' : 'border-gray-200 dark:border-slate-700'">
                        ...
                        <div>
                            <label class="block text-xs font-extrabold text-gray-500 dark:text-slate-400 mb-2">勾选目标通知渠道</label>
                            <div class="grid grid-cols-2 gap-2 p-3 border rounded-xl bg-gray-50 dark:bg-slate-900 dark:border-slate-700 max-h-40 overflow-y-auto">
                                <label v-for="(ch, idx) in config.channels" :key="idx" class="flex items-center space-x-2 cursor-pointer text-xs font-medium p-1 hover:bg-gray-200/50 dark:hover:bg-slate-800 rounded">
                                    <input type="checkbox" :value="ch.name" v-model="batchNotifyChannels" class="rounded border-gray-300">
                                    <span class="truncate">{{ ch.name }}</span>
                                </label>
                                <div v-if="config.channels.length === 0" class="text-xs text-gray-400 col-span-2 py-2 text-center">暂无可用渠道，请先去添加</div>
                            </div>
                        </div>
                    </div>
                    <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-3">
                        <button @click="showBatchModal = false" class="px-5 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 text-xs font-bold hover:bg-gray-100 dark:hover:bg-slate-700 transition">取消</button>
                        <button @click="confirmBatchAssign" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-md transition">确认批量生效</button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="showTaskModal" class="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all duration-200 animate-fade-in">
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full border border-gray-200 dark:border-slate-700 overflow-hidden">
                    <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
                        <h3 class="font-black text-base text-indigo-600 dark:text-indigo-400 flex items-center gap-2"><span>✏️</span> 编辑保活任务</h3>
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-3">
                        ...
                        <button @click="confirmEditTask" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-md transition">保存修改</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        createApp({
            setup() {
                ...
                const doLogin = async () => {
                    ...
                    try {
                        ...
                        if (data.status === 'ok') {
                            ...
                            alert('❌ 登录验证失败：账号或访问密码不正确！');
                        }
                    } catch(e) { alert('网络请求异常：' + e.message); }
                    finally { isLoggingIn.value = false; }
                };

                const doLogout = async () => {
                    if (!confirm('确定要退出当前面板的管理登录吗？')) return;
                ...
                const openBatchChannelModal = () => {
                    batchNotifyChannels.value = [];
                    batchMode.value = 'overwrite';
                ...
                const confirmBatchAssign = () => {
                    selectedTaskIndices.value.forEach(idx => {
                        ...
                        if (batchMode.value === 'overwrite') {
                            t.notifyChannels = [...batchNotifyChannels.value];
                        } else if (batchMode.value === 'append') {
                            const merged = new Set([...(t.notifyChannels || []), ...batchNotifyChannels.value]);
                            t.notifyChannels = Array.from(merged);
                        }
                    });
                ...
                const batchRemoveTasks = () => {
                    ...
                    const sortedDescending = [...selectedTaskIndices.value].sort((a, b) => b - a);
                    sortedDescending.forEach(idx => config.value.tasks.splice(idx, 1));
                    selectedTaskIndices.value = [];
                };
                ...
                onMounted(() => {
                    ...
                    checkAuthSession(); // 页面装载首飞校验会话 Cookie
                });

                return { 
                    isDark, currentTab, hasUnsavedChanges, toggleTheme, formatTime, formatLogTime,
                    isLoggedIn, isLoggingIn, loginForm, doLogin, doLogout,
...
async function verifySessionToken(secret, token, expectedUser) {
    ...
    try {
        ...
        // Cookie 有效期设定为 7 天 (604800 秒)
        if (Date.now() - parseInt(ts) > 604800000) return false;
...
export default {
    async fetch(request, env, ctx) {
        ...
        // 3. 后台所有私有 API 数据网关的会话验签门神
        if (url.pathname.startsWith('/api/')) {
            const sessionCookie = getCookie(request, 'KA_SESSION');
            const isValid = await verifySessionToken(adminPass, sessionCookie, adminUser);
            
            if (url.pathname === '/api/check-session') {
                return isValid ? new Response('ok') : new Response('unauthorized', { status: 401 });
            }

            if (!isValid) {
                return new Response('{"error":"Session Expired or Invalid"}', { status: 401, headers: { 'Content-Type': 'application/json' } });
            }
    ...
    async scheduled(event, env, ctx) {
        ...
        for (let task of config.tasks) {
            ...
            if (now - (task.lastCheck || 0) >= intervalMs) {
                ...
                try {
                    const res = await fetch(task.url, { method: 'GET', headers: { 'User-Agent': 'Cloudflare-KeepAlive-Pro/12.0' }, cf: { cacheTtl: 0 } });
                    isSuccess = res.ok;
